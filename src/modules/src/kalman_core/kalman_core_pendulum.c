...
#include "kalman_core_pendulum.h"
...
static void predictDt(kalmanCoreData_t* this, const kalmanCoreParams_t *params, Axis3f *acc, Axis3f *gyro, Axis3f *gyro_prev, float dt, bool quadIsFlying)
{
  ...
  if (quadIsFlying) // only acceleration in z direction // CHANGED THIS
  {
    ...
    dx = this->S[KC_STATE_PX] * dt + xacc * dt2 / 2.0f; // UPDATED
    dy = this->S[KC_STATE_PY] * dt + yacc * dt2 / 2.0f; // UPDATED
    ...
    this->S[KC_STATE_PX] += dt * (xacc + gyro->z * tmpSPY - gyro->y * tmpSPZ - GRAVITY_MAGNITUDE * this->R[2][0]); // CHANGED
    this->S[KC_STATE_PY] += dt * (yacc - gyro->z * tmpSPX + gyro->x * tmpSPZ - GRAVITY_MAGNITUDE * this->R[2][1]); // CHANGED
